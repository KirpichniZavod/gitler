...
class WebSocketHandler {
  ...
  setupWebSocket() {
    ...
    console.log(`🔌 WebSocket сервер создан: ${!!this.wss}`)
    console.log(`🔌 Количество клиентов: ${this.wss.clients.size}`)

    // Логирование событий WebSocket сервера
    this.wss.on("error", (error) => {
      console.error("❌❌❌ ОШИБКА WSS СЕРВЕРА ❌❌❌")
    ...
    setInterval(() => {
      console.log(
        `📊 WSS СТАТИСТИКА: Клиентов: ${this.wss.clients.size}, Пользователей: ${this.users.size}, Комнат: ${this.rooms.size}`,
      )
    }, 10000)
    ...
    this.wss.on("connection", (ws, req) => {
      ...
      if (ws.readyState !== WebSocket.OPEN) {
        console.log(`⚠️ WebSocket не в состоянии OPEN: ${ws.readyState}`)
      }
